...
This project uses a Packer template to provision a prebaked AMI containing the Apache web server based off of Amazon Linux. The AMI is then passed to a CloudFormation template as a parameter value. The CloudFormation template creates a VPC with one public subnet, an internet gateway, a NACL with ports 22,80,443 open, an EC2 Instance based off of the Packer AMI that is bootstrapped to provide all updates and copies over a preconfigured index.html file , and an EC2 security group allowing ports 22,80,443.
